...
public class LargeFileReader {
  ...
  private LargeFileReader(
      File file, IHandle handle, String charset, int bufferSize, int threadSize) {
  ...
  public void start() {
    ...
    // 需要再此处加上判断，上述线程是否完毕，然后结束操作shutdown
  }
    ...
    public void run() {
      try {
        MappedByteBuffer mapBuffer =
            rAccessFile.getChannel().map(MapMode.READ_ONLY, start, this.sliceSize);
  ...
  public static class Builder {
    ...
    public LargeFileReader build() {
      return new LargeFileReader(
          this.file, this.handle, this.charset, this.bufferSize, this.threadSize);
    }
  }
}
